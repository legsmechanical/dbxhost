...
| ~~**Q4**~~ | ✅ **ANSWERED 08-04: C1** — rewrite at launch, archive originals, **never restore on exit**, **plus a confirmation before an existing set's routing is overwritten** (no-op sets must not prompt). Design B rejected. Full analysis + the prompt-placement open question + a correction to the "we've written to sets before" assumption: [`DBSA_SET_MODEL.md`](DBSA_SET_MODEL.md). | B8/B9/B10 |
