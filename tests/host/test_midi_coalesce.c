...
int main(void) {
    ...
    {
        uint8_t b[64]; memset(b, 0, sizeof(b));
        put(b, 0, 0x0B, 0xB0, 71, 3);      /* +3 */
        put(b, 1, 0x0B, 0xB0, 71, 128 - 1);/* -1 */
        shadow_coalesce_relative_ccs(b, sizeof(b));
    ...
    {
        uint8_t b[64]; memset(b, 0, sizeof(b));
        put(b, 0, 0x0B, 0xB0, 72, 5);
        put(b, 1, 0x0B, 0xB0, 72, 128 - 5);
        shadow_coalesce_relative_ccs(b, sizeof(b));
        check("net-zero movement is dropped entirely", packets(b, sizeof(b)) == 0);
    }

    /* 4. Different knobs are independent. */
    {
        uint8_t b[64]; memset(b, 0, sizeof(b));
        put(b, 0, 0x0B, 0xB0, 71, 1);
        put(b, 1, 0x0B, 0xB0, 72, 1);
        put(b, 2, 0x0B, 0xB0, 71, 1);
        shadow_coalesce_relative_ccs(b, sizeof(b));
        check("two knobs stay two packets", packets(b, sizeof(b)) == 2);
        check("knob 71 summed to +2", b[3] == 2);
        check("knob 72 untouched at +1", b[7] == 1);
    }
    ...
    {
        uint8_t b[64]; memset(b, 0, sizeof(b));
        for (int i = 0; i < 15; i++) put(b, i, 0x0B, 0xB0, 73, 9);  /* +135 */
        shadow_coalesce_relative_ccs(b, sizeof(b));
