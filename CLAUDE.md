...
- **Two fork-only JS bindings: `host_vol_block` and `host_edit_cc_block`.** These are the fork's *entire* JS-visible API delta — everything else it adds is behavioural, driven by `capabilities.*` flags a stock host simply never reads. Both are documented in `docs/API.md` marked `[FORK-ONLY]`; `host_edit_cc_block` previously existed only in `src/` with no doc entry at all, which is exactly how a rebase drops a binding silently while the module that calls it keeps quietly falling back. **If a rebase or upstream sync removes either, the davebox module loses a feature with no error** — it gates on `typeof`, so absence is indistinguishable from a stock install. Consumers: `ui/ui_engine.mjs:131` and `ui/ui_sound.mjs:2190` in the module repo.
