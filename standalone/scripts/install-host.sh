...
# --- help content ------------------------------------------------------------
# The browser Help page renders markdown from $DBX_DIR/help/, generated by
# splitting the manual into one page per chapter.
#
# Generated HERE rather than in build-host.sh on purpose: it is pure text
# processing that costs milliseconds and needs no toolchain, and `--no-build`
# must still ship docs matching the manual in the working tree — a stale help/
# from an earlier build is exactly the kind of drift the generator exists to
# prevent. Non-fatal: a docs failure is not a reason to block a host deploy.
say ""; say "--- generating help content from the manual"
