...
import {
    ...
    /* The browser's BEHAVIOUR, moved down beside its state so a second consumer
     * inherits it instead of rediscovering the gap on hardware. See the live
     * preview section of filepath_browser.mjs for the two-member io contract. */
    initFilepathPreview,
    armFilepathPreview,
    tickFilepathPreview,
    syncFilepathPreviewToSelection,
    commitFilepathSelection,
    finishFilepathPreview
...
function filepathBrowserJog(delta) {
    ...
    armFilepathPreview(filepathBrowserState);
...
/* How the filepath browser reads and writes a parameter in the hierarchy
 * editor. The whole of the browser's behaviour — the live audition, the
 * module's browser_hooks, and putting back what a cancel borrowed — lives in
 * the shared library now; this object is the only part that is the host's own.
 *
 * ⚠ `hierEditorSlot` is read AT CALL TIME, never captured when the browser
 * opens: a browser can outlive the slot it was opened on, and a write that went
 * to the captured slot would land an edit on a chain the user never opened. */
const FILEPATH_BROWSER_PARAM_IO = {
    getParam(key) {
        return getSlotParam(hierEditorSlot, key);
    },
    setParam(key, value) {
        return setSlotParam(hierEditorSlot, key, value);
    }
};
 ...
 * (Shift+Vol+jog click to launch, Shift+long-press Step13 to resume). The
 * release lands during the blackout when the module is not yet receiving
 * MIDI, so it is discarded and both the host and the module stay latched in
 * shift-mode for the whole session — upstream issue #191.
 *
 * It does not present as one bug. Upstream saw a knob editing a base value
 * instead of writing a parameter lock, a pad loading the wrong machine, and a
 * pad opening the sample browser: three unrelated hardware reports, one
 * cause. davebox is intensely shift-modal, so the same latch reaches dozens
 * of branches across ui_input_cc / ui_dialogs / ui_drummodel.
 *
 ...
 *
 * Width of the window is the module's own init(), which for davebox is long.
 *
...
/* Move FX per-set persistence — mirrors the Send FX pair above. Each Move FX
...
/* Move FX hierarchy editor — mirrors enterSendFxHierarchyEditor. The compKey
...
function openHierarchyFilepathBrowser(key, meta) {
    ...
    /* Arms live preview and runs the module's on_open hook — BEFORE the first
     * refresh, because that hook may set the very params (start_path) the
     * listing is read from. */
    initFilepathPreview(filepathBrowserState, effectiveMeta, fullKey, currentVal,
                        prefix, FILEPATH_BROWSER_PARAM_IO);
    filepathBrowserKnobAccum = 0;

    refreshFilepathBrowser(filepathBrowserState, FILEPATH_BROWSER_FS);

    syncFilepathPreviewToSelection(filepathBrowserState, FILEPATH_BROWSER_PARAM_IO);
...
function closeHierarchyFilepathBrowser() {
    /* Commit the choice, or put back the parameter and everything a hook
     * borrowed. */
    finishFilepathPreview(filepathBrowserState, FILEPATH_BROWSER_PARAM_IO);
...
function createCanvasRuntimeContext() {
    ...
    return {
         ...
         * wrapper is built in schwung_host.c, a different JSContext. So the old
         * guard did not read as falsy, it THREW a ReferenceError on an
         * undeclared identifier, which disabled the overlay for the session and
         * left a canvas page drawing chrome around an empty body. */
...
function canvasOverlayHookResult(hookName, payload) {
    ...
    /* ONE STRIKE, taken from upstream with the rest of this contract.
     *
     * This used to record the error, log, and carry on -- so a throwing overlay
     * threw on EVERY FRAME, forever, at 60Hz. The error was visible in the log
     * and the flood was not. A fresh canvasRuntime is built on every open, so
     * the flag clears itself when the screen is reopened. */
 ...
 * and Back as the close gesture. For a visualiser that is right. For anything
 * NESTED it is fatal, because the one gesture that means "enter" is the one
 * spent on "leave", so a file browser or a settings menu cannot be built as a
 * canvas at all.
 *
 * `enterable: true` says the module has navigation inside it. Upstream's
 * contract; see docs/CANVAS_PAGES.md there. This replaces the fork's own
 * `canvas_takes_click`, which asked for the click rather than describing the
 * screen.
 */
...
function tickCanvasPreview() {
     ...
     *
     * The knob grid reconciles `pad_observe` from the module's contract, and
     * opening a canvas LEAVES the grid -- so a module-drawn browser could not
     * tell which pad you pressed, on the one screen where filling a pad is the
     * entire job.
     *
     * OBSERVE, NEVER BLOCK: the shim adds a publish and does not `continue`, so
     * the pad still plays. That is what you want while auditioning.
     *
     * ⚠ RESTATED EVERY TICK rather than raised on open: the shim drops the flag
     * on its own authority when the shadow display closes, so a JS mirror goes
     * stale and the feature dies silently for the rest of the session.
     */
...
function handleSelect() {
    ...
    switch (view) {
            ...
            if (inMasterPresetPicker) {
                ...
                if (selectedComp.key === "settings") {
                    ...
                    if (moduleData && moduleData.module) {
                         ...
                         * dedicated send hierarchy editor; Move FX its own;
                         * master uses its own. */
        ...
        case VIEWS.CANVAS:
             ...
             * one, but this is also the screen reader's and the remote UI's
             * select, and those would close a browser mid-navigation. */
        ...
        case VIEWS.FILEPATH_BROWSER:
            ...
            }
            {
                ...
                if (result.action === "open") {
                    ...
                    const fullKey = buildHierarchyParamKey(key);
                    commitFilepathSelection(filepathBrowserState, result.value);
...
(function populateCtx() {
     ...
     * on touch and on turn and caches the answer, including a null. There is no
     * module script resident while the grid is up and shadow_load_ui_module
     * has no cache of its own, so a load from the draw path would evaluate the
     * script on every frame of a knob turn.
     */
...
globalThis.init = function() {
    ...
    restoreSendFxFromFiles();
    /* Move FX likewise has no shim-side boot restore. */
    restoreMoveFxFromFiles();
...
globalThis.tick = function() {
    ...
    tickFilepathPreview(filepathBrowserState, FILEPATH_BROWSER_PARAM_IO);
...
globalThis.onMidiMessageInternal = function(data) {
    ...
    if ((view === VIEWS.CANVAS || canvasInCorun) && (status & 0xF0) === 0xB0) {
        ...
        if (d1 === MoveBack && d2 > 0) {
            /* ⭐ BACK IS THE EXIT CONTRACT, and it is what makes handing over the
             * click safe:
             *
             *   handleBack() === true   "I went up a level"    -> stay inside
             *   anything else           "I am at my top level" -> the host closes
             *
             * So a module implements a way UP, never a way OUT. Offered only to
             * an enterable canvas -- a visualiser has no levels to climb, and
             * asking it would make Back's meaning depend on a hook nobody
             * declared. A throwing hook answers CANVAS_HOOK_ABSENT, so it can
