 ...
 * tail is SIGBUS. Deploy the shim and shadow_ui together, as install-sa.sh
 * does. Growing is the safe direction for the other order too.
 *
 * ⚠ FORK NOTE: this also raises davebox's state-blob ceiling, which is the
 * PARAM TRANSPORT and not `state_buf` -- the limit that once truncated a
 * project mid-note and reloaded it smaller. The chunked writer stays; this
 * just moves the wall. */
...
typedef struct shadow_control_t {
     ...
     * never consumed by the C side. Overloading it with an exit-time "let Move
    ...
    /* RESERVED. Was native_repaint_on_exit (backport 25f73f81), itself reusing
     * the retired skipback_require_volume byte. That request told the audio
     * side to DISCARD its LED snapshot on the overtake->0 transition, on
     * upstream's premise that Move repaints the surface itself.
     *
     * ⭑ THE PREMISE DOES NOT HOLD HERE. Device-confirmed 2026-08-30: after a
     * hold-Back suspend the OLED went native correctly and THE PADS STILL SHOWED
     * DAVEBOX, because the discard is exactly "do not repaint" and Move never
     * does on this path. The request was removed, which left the byte, the C
     * branch and the JS binding with no caller, and Josh called the revert.
     *
     * Kept as a reserved byte so CONTROL_BUFFER_SIZE and the SHM layout are
     * untouched -- the shim creates that SHM and shadow_ui maps it through the
     * same macro, and shadow_control_size_check below is what proves it. */
    volatile uint8_t reserved_was_native_repaint_on_exit;
