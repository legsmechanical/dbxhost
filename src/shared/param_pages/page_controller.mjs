...
export function createController(io = {}) {
     ...
     * state".
     *
     * ⚠ CORRECTED (#451): this used to say "a preset Load does not need this
     * only because it comes back through the browser". True of a preset VIEW,
     * which re-enters the grid and so replans and re-warms — and FALSE of an
     * in-grid preset PAGE, which never leaves. That page drops the same two
     * maps directly; see stepPreset. A recall needs it for the original
     * reason: it happens under your hands with no re-entry at all.
     *
    ...
    function stepPreset(delta) {
        ...
        st.index = next;
        /*
         * A PENDING KNOB WRITE BELONGS TO THE PRESET YOU ARE LEAVING (#451).
         *
         ...
         * preset that had just replaced it — one knob of the old sound stamped
         * onto the new one.
         */
        flushDueWritesUnconditionally();
        ...
        armContractSettle();
        /*
         * AND EVERY CACHED VALUE NOW DESCRIBES THE PRESET YOU JUST LEFT (#451).
         *
         ...
         * Nothing reads on the draw path — values arrive on touch-down, on the
         * rotation, or in the entry warm — and `onKnobTurn` steps FROM the
         * cached value, so the first knob move after choosing a preset departed
         * from the PREVIOUS preset's number and wrote it back over the one just
         * loaded. Reported from hardware upstream as "after changing a preset
         * the knob turns from the last value, not the preset's".
         *
         ...
         * would pay a page of reads per detent for values nobody is looking at,
         * and the module may still be loading the preset it was asked for.
         * Dropping the maps costs nothing, and the read cursor and the entry
         * warm refill them when a knobs page is actually reached.
         *
         * ⭑ Same defect as the snapshot recall, through a different door — see
         * revalue(), whose comment asserted a preset load did not need it
         * because it "comes back through the browser". True of a preset VIEW,
         * which re-enters and replans; false of an in-grid preset PAGE, which
         * never leaves.
         */
        s.values = Object.create(null);
        s.knobStates = Object.create(null);
