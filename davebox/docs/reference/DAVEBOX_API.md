...
| `tN_lL_import` | set | `"<flags> <res_idx> <len>\|a tick vel gate;…"` | Phrase library: one drum lane of the active clip, one undo unit (whole drum clip). Sets that lane's tps/length only; hits take the lane's `midi_note`; flags bit0 = replace. Ends any preview on the track first (undo holds the original). In phase with the master clock while playing. Refused while recording. |
| `tN_lanes_import` | set | `"<flags> <res_idx> <len>\|L7;a tick vel gate;…;L9;a …"` | Phrase library: several drum lanes of the active clip as ONE undo unit; only the named lanes change (each gets the grid/length given); flags bit0 = replace each named lane. Otherwise as `tN_lL_import`. Drum tracks only. |
| `tN_audclip` | set | `"<res_idx> <len> <lane>\|…"` or `"off"` | Phrase library preview, swapped in on the next beat (at once while stopped). lane `-1`: the track's current clip, `a tick pitch vel gate;…`; lane `L`: one drum lane, `a tick vel gate;…`; lane `-2`: several drum lanes (≤ 8), `L7;a …;L9;a …`. A re-staging's dropped lanes are restored on the same beat; `off` restores all. Never saved (a save writes the originals), never undone, refused while recording; a project load drops it. |
...
| `tN_pa_hold` | `"<target>"` | a hand is dialling a LOCK on this target: playback leaves it alone until `pa_live_end`, and nothing is recorded whatever Record says |
