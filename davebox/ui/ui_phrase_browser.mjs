 ...
 *              2026-09-23: "this note from the sequence goes on this drum track
 *              pad (or melodic track NOTE) by tapping the relevant pad".
 *
...
import { White } from '/data/UserData/schwung/shared/constants.mjs';
...
function replacing() {
    ...
    if (!p) return false;
    if (PB.drum) return [...drumLanes().keys()].some(l => GS.drumLaneHasNotes[t][l]);
...
function commit() {
    ...
    const where = PB.drum ? (PB.voices.length > 1 ? PB.voices.length + ' PADS' : 'PAD ' + (PB.assign[0] + 1))
...
/* A pad (0-31, bottom-left first): put the selected instrument there. */
export function pbPadTap(i) {
    if (!PB || PB.confirm || !PB.voices.length) return;
    const t = PB.track;
    let target = -1;
    if (PB.drum) {
        const col = i % 8, row = Math.floor(i / 8);
        if (col >= 4) return;
        const lane = (GS.drumLanePage[t] | 0) * 16 + row * 4 + col;
        if (lane < 0 || lane >= DRUM_LANES) return;
        target = lane;
    } else {
        const n = GS.padNoteMap[i];
        if (n === 0xFF || n == null) return;
        target = Math.max(0, Math.min(127, (n | 0) + (GS.trackOctave[t] | 0) * 12));
    }
    PB.assign[PB.voiceSel] = target;
    if (PB.voices.length > 1) PB.voiceSel = (PB.voiceSel + 1) % PB.voices.length;
    GS.screenDirty = true;
}

/* Every pad's colour while the screen is up, or null to leave the pads to the
 * track (a melodic phrase: nothing to assign). */
export function pbPadColors() {
    if (!PB || !PB.voices.length) return null;
    const t = PB.track, out = new Array(32).fill(LED_OFF);
    const sel = PB.assign[PB.voiceSel];
    const blink = (nowMs() % 400) < 200;
    for (let i = 0; i < 32; i++) {
        let target;
        if (PB.drum) {
            const col = i % 8, row = Math.floor(i / 8);
            if (col >= 4) continue;
            target = (GS.drumLanePage[t] | 0) * 16 + row * 4 + col;
        } else {
            const n = GS.padNoteMap[i];
            if (n === 0xFF || n == null) continue;
            target = (n | 0) + (GS.trackOctave[t] | 0) * 12;
        }
        if (target === sel) out[i] = blink ? White : TRACK_COLORS[t];
        else if (PB.assign.includes(target)) out[i] = TRACK_COLORS[t];
        else if (PB.drum && GS.drumLaneHasNotes[t][target]) out[i] = TRACK_DIM_COLORS[t];
    }
    return out;
}
...
/* The header names the destination; with several instruments, the selected
 * one and where it goes ("(2) OPEN > PAD 11"), which the pads change. */
function header() {
    const t = PB.track;
    if (PB.voices.length > 1)
        return '(' + (t + 1) + ') ' + voiceLabel(PB.voiceSel) + ' > ' + (PB.drum ? 'PAD ' : '') + targetLabel(PB.assign[PB.voiceSel]).replace(/^P/, '');
...
function drawRoll(y, h) {
    ...
    if (PB.voices.length > 1 && r.rows > 1) {
        const ry = y + h - 1 - Math.floor(PB.voiceSel * (h - 1) / (r.rows - 1));
        fill_rect(0, ry, 2, 1, 1);
    }
}
...
export function pbRender(touchedIdx, shift) {
    ...
    if (PB.picker && touched < 0) drawPicker();
}
