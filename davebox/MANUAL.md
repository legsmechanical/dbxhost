<!-- FROZEN-BANNER-START -->
> 📕 **This manual is frozen, and it is not the one being developed.**
>
> It documents dAVEBOx running as an ordinary tool inside official Schwung, as of
> its final release under that model. That version still works and is unchanged —
> nothing here has been withdrawn — but development has moved to **dAVEBOx SA**,
> which runs dAVEBOx as its own session on its own build of the host.
>
> **For current dAVEBOx, read [`MANUAL-SA.md`](MANUAL-SA.md).** It covers four
> insert effects per chain instead of two, two send buses, booting straight into
> the sequencer, and handing the device back to official Schwung on exit.
>
> *Maintainers: do not edit this file and do not port changes into it. The living
> source is `docs/working/MANUAL-SA.draft.md`.*
<!-- FROZEN-BANNER-END -->
